...
The path to writing better PRs starts with being able recognize the difficult ones. Here are a few key identifiers I look for.
...
The smaller a PR is, the easier it will be for a reviewer to stay focused. Consider the amount of thought needed for 1 line change PR and a 50+ file PR. Which one are you more likely to review immediately? Which one are you less likely overlook an unwanted code change or bug? Keeping a PR focused on specific goals is an art form that significantly helps keep reviewers focused. If there are features that can be broken out into smaller PRs, the time saved reviewing can be worth it.
